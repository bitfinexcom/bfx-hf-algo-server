...
class Server extends EventEmitter {
   ...
   * @param {AOAdapter} args.adapter - exchange API adapter instance
   ...
   *
   * @param {module:bfx-hf-algo.AOHost} aoHost - algo host
   */
  setAlgoHost (aoHost) {
    this.clearAlgoHost()
    this.host = aoHost
  ...
  /**
   * @param {module:bfx-hf-algo.AOInstance} instance - algo order instance
   * @private
   */
  onAOStart (instance) {
  ...
  /**
   * @param {object} instance - algo order instance
   * @private
   */
  onAOStop (instance) {
    const { state = {} } = instance
   ...
   * @param {number} mid - incoming notification ID, used to generate response ID
   * @param {string} type - incoming notification type, replicated on outgoing success/error notification
